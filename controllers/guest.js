...
export async function handleGetAllBlogs(_req, res) {
    try {
...
export async function handleGetBlog(req, res) {
    try {
        const blog = await queries.getBlog(req.params.id);
        if (!blog) {
...
export async function handleGetBlogComments(req, res) {
    try {
        const comments = await queries.getBlogComments(req.params.id);
        res.json({ comments });
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: 'Internal server error' });
...
export async function handleNewComment(req, res) {
    ...
    try {
        const comment = await queries.insertNewComment(req.body, req.params.id);
