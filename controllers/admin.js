...
export function handleSucessLogin(_req, res) {
    res.status(200).json({ message: 'success' });
}

export async function handleGetAllBlogs(req, res) {
    try {
        const blogs = await queries.getAllBlogs(req.user);
        res.json({ blogs });
    } catch (error) {
...
export async function handleGetBlog(req, res) {
    try {
        const blog = await queries.getBlog(req.params.id, req.user);
        if (!blog) {
...
export async function handleGetBlogComments(req, res) {
    try {
        const comments = await queries.getAllBlogComments(req.params.id, req.user);
        res.json({ comments });
    } catch (error) {
