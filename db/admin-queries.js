...
export async function getAllBlogs(user) {
    ...
    `;
    const result = await pool.query(sql, [isAdmin, user.id]);
    return result.rows;
}
...
export async function getAllBlogComments(blog_id, user) {
    ...
    `;
    const result = await pool.query(sql, [blog_id, isAdmin, user.id]);
    return result.rows;
}
...
export async function getBlog(id, user) {
    ...
    `;
    const result = await pool.query(sql, [id, isAdmin, user.id]);
...
export async function insertBlog(blog, user_id) {
    ...
    `;
    const result = await pool.query(sql, [blog.title, blog.content, user_id, blog.published || false]);
    return result.rows[0];
}
