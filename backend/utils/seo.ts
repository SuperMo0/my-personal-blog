import type { BlogRow, BlogSummaryRow } from '../types.ts';
...
function summarise(html: string | null | undefined, limit = 160): string {
...
export function articleMeta(blog: BlogRow): Meta {
    return {
...
export function staticMeta(pathname: string): Meta | null {
...
export function renderSitemap(blogs: BlogSummaryRow[]): string {
    const entries = [
        { loc: `${SITE_URL}/`, priority: '1.0', lastmod: null as string | null },
        { loc: `${SITE_URL}/about`, priority: '0.8', lastmod: null as string | null },
        { loc: `${SITE_URL}/cv`, priority: '0.6', lastmod: null as string | null },
        ...blogs.map((blog) => ({
