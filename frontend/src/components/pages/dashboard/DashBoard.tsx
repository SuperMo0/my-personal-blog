...
import { GoGlobe, GoHeart, GoPencil, GoTrash, GoXCircle } from 'react-icons/go';
import { Link } from 'react-router';
import { useAuth } from '../../../auth/AuthContext';
import api from './../../../utils/Api';
import ContributionsManager from '../../contributions-manager/ContributionsManager';
...
export default function DashBoard() {
    ...
    useEffect(() => {
        async function getArticles() {
            try {
                const [result, ok] = await api<{ blogs: Article[] }>('/admin/blogs', {});
    ...
    async function handleStatus(id: number, currentStatus: boolean) {
        const body = { published: !currentStatus };
    ...
    return (
        <div className="wrapper py-10">
                                    ...
                                    <tr key={a.id} className="hover:bg-(--bg-primary) transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <button
                                                ...
                                                disabled={readOnly}
                                                className={`${readOnly ? 'cursor-default' : 'cursor-pointer'} px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wider transition-colors disabled:opacity-100 ${
                                                    a.published
            ...
            <ContributionsManager readOnly={readOnly} />
        </div>
    );
}
