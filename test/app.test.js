...
function graphQLCalendarResponse(days) {
    return new Response(
        JSON.stringify({
            data: {
                user: {
...
function createGitHubFetch(figures, { calendarDays, calendarResponse } = {}) {
...
describe('read-only demo access', () => {
    ...
    test('the configured Express app can be driven over HTTP without listening', async () => {
        ...
        assert.deepEqual(
            response.body.blogs.map((blog) => blog.title),
            ['Published guide'],
        );
    });
    ...
    test('lets an admin create, edit, publish, and delete articles', async () => {
        ...
        assert.equal(
            list.body.blogs.some((blog) => blog.id === newArticle.id),
            false,
        );
    });

    test('refuses missing, malformed, wrongly formatted, and expired tokens', async () => {
        const expiredToken = jwt.sign({ id: 100, email: 'owner@example.com', role: 'admin' }, process.env.SECRET, {
            algorithm: 'HS256',
            expiresIn: -1,
        });
